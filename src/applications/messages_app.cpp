...
{
    if (!lv_obj_is_valid(EmbeddedUI_t.messages))
    {
        EmbeddedUI_t.messages = lv_obj_create(lv_screen_active());

        lv_obj_set_size(EmbeddedUI_t.messages, TFT_WIDTH - 20, TFT_HEIGHT - 60);
        lv_obj_center(EmbeddedUI_t.messages);

        /*Create a keyboard to use it with an of the text areas*/

        /*Create a text area. The keyboard will write here*/
        lv_obj_t *ta1;
        ta1 = lv_textarea_create(EmbeddedUI_t.messages);
        ...
        lv_obj_t *kb = lv_keyboard_create(EmbeddedUI_t.messages);
        ...
        EmbeddedUI_t.close_btn = lv_button_create(EmbeddedUI_t.messages);
        ...
        lv_obj_align(EmbeddedUI_t.close_btn, LV_ALIGN_TOP_RIGHT, 0, -5);
