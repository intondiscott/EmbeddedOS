...
static void create_clock_page(lv_event_t *e)
{
    ...
    if (!lv_obj_is_valid(EmbeddedUI_t.datetime_label))
    {

        EmbeddedUI_t.datetime_label = lv_obj_create(lv_screen_active());
        lv_obj_t *label = lv_label_create(EmbeddedUI_t.datetime_label);

        lv_obj_t *military_switch = lv_switch_create(EmbeddedUI_t.datetime_label);
        lv_obj_set_size(military_switch, 90, 50);
        lv_obj_t *military_label = lv_label_create(EmbeddedUI_t.datetime_label);
        EmbeddedUI_t.close_btn = lv_button_create(EmbeddedUI_t.datetime_label);
        ...
        lv_obj_add_event_cb(
            ...
            (void *)military_label);
        lv_obj_align(EmbeddedUI_t.close_btn, LV_ALIGN_BOTTOM_MID, 0, 20);
        ...
        lv_label_set_text_fmt(label,
                              ...
                              time_vals->day,
                              time_vals->year);
        lv_obj_set_style_radius(EmbeddedUI_t.datetime_label, 40, LV_PART_MAIN);
        lv_obj_set_style_pad_all(EmbeddedUI_t.datetime_label, 20, LV_PART_MAIN);
        lv_obj_set_size(EmbeddedUI_t.datetime_label, TFT_WIDTH - 0, TFT_HEIGHT - 0);
        lv_obj_center(EmbeddedUI_t.datetime_label);
        EmbeddedUI_t.datetime_timer = lv_timer_create(update_clock_label, 1000, label);
