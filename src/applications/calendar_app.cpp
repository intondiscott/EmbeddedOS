...
{
    if (!lv_obj_is_valid(EmbeddedUI_t.calendar))
    {
        EmbeddedUI_t.calendar = lv_obj_create(lv_screen_active());
        lv_obj_t *calendar = lv_calendar_create(EmbeddedUI_t.calendar);
        EmbeddedUI_t.close_btn = lv_button_create(EmbeddedUI_t.calendar);
        ...
        lv_obj_set_size(EmbeddedUI_t.calendar, TFT_WIDTH - 20, TFT_HEIGHT - 60);
        lv_obj_set_size(calendar, 220, TFT_HEIGHT - 100);
        lv_obj_center(EmbeddedUI_t.calendar);
