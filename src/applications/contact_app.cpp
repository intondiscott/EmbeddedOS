...
{
    if (!lv_obj_is_valid(EmbeddedUI_t.contact))
    {
        EmbeddedUI_t.contact = lv_obj_create(lv_screen_active());

        lv_obj_t *label = lv_label_create(EmbeddedUI_t.contact);
        EmbeddedUI_t.close_btn = lv_button_create(EmbeddedUI_t.contact);
        ...
        lv_obj_align(EmbeddedUI_t.close_btn, LV_ALIGN_TOP_RIGHT, 0, 0);
        lv_obj_set_size(EmbeddedUI_t.contact, TFT_WIDTH - 20, TFT_HEIGHT - 60);
        lv_obj_center(EmbeddedUI_t.contact);
