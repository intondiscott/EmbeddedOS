...
{
  if (!lv_obj_is_valid(EmbeddedUI_t.phone))
  {
    EmbeddedUI_t.phone = lv_obj_create(lv_screen_active());
    lv_obj_t *label = lv_label_create(EmbeddedUI_t.phone);
    EmbeddedUI_t.close_btn = lv_button_create(EmbeddedUI_t.phone);
    ...
    lv_obj_set_size(EmbeddedUI_t.phone, TFT_WIDTH - 20, TFT_HEIGHT - 60);
    lv_obj_center(EmbeddedUI_t.phone);
